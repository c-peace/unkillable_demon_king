...
**Evaluator-compatible vertical slice implemented; live development L2/MCP path verified and post-retrieval generation hardened.** The service implements the required HTTP/Docker boundary, full-history L2 generation, two-stage MCP retrieval, citation provenance, bounded failure handling, and feature-flagged advanced paths. Standard non-streaming L2 tool calls, dynamic MCP discovery/calls, live citation capture, and a complete L2-to-MCP-to-L2 request have succeeded. The retrieval bridge now deterministically auto-follows indexed guideline/HIRA node discovery into page reads when document and page metadata are present, which reduces repeated `index_get_relevant_nodes` loops. The L2 adapter also normalizes text-encoded `<tool_call>...</tool_call>` messages into standard tool calls, and generation now injects an explicit "Retrieval is complete" system prompt once retrieval budget is exhausted so legacy tool-call markup does not leak into the final assistant answer. Patient Simulator, dashboard evaluation, and evaluation-environment secret/connectivity behavior remain unverified.
